...
def get_feed(request):
    ...
    query = DBSession.query(ClassificationData, SpotCheckJob.job_type).join(SpotCheckJob).filter(ClassificationData.job_id == int(job_id))
    ...
    for record in query.all():
        rec = record.ClassificationData
        product = {'url': rec.url, 'title': rec.title, 'breadcrumb': rec.breadcrumb, 'categorypath1': rec.categorypath1,
                   'categorypath2': rec.categorypath2, 'pentos_id': rec.pentos_id, 'id': rec.id, 'job_id': rec.job_id, 'job_type':record.job_type}
        data.append(product)
    return data
...
def spot_check(request):
    ...
    domain = request.params.get('domain')
    record_id = request.params.get('record_id')
    return {'user': user, 'job_id': spot_check_job_id, 'domain': domain, 'only_unmarked': only_unmarked,
            'record_id': record_id}
